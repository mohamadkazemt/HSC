...
class GymReferralUITests(TestCase):
    ...
    def test_authorized_gym_list_create_edit_and_inactive_status(self):
        ...
        created = self.client.post(reverse("gym_referrals:gym_create"), {"name": "باشگاه آزادی", "code": "AZD", "phone": "", "address": "تهران", "is_active": "on"})
        gym = Gym.objects.get(code="AZD")
        self.assertRedirects(created, reverse("gym_referrals:gym_detail", args=(gym.pk,)))
        self.client.post(reverse("gym_referrals:gym_edit", args=(gym.pk,)), {"name": gym.name, "code": gym.code, "phone": "", "address": gym.address})
        gym.refresh_from_db()
