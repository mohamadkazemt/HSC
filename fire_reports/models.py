...
class VehicleChecklist(models.Model):
    """
    مدل چک‌لیست وضعیت خودرو که می‌تواند چندین خودرو را در یک گزارش پشتیبانی کند
    """
    STATUS_CHOICES = [
        ('suitable', 'مناسب'),
        ('unsuitable', 'نامناسب'),
    ...
    fire_report = models.ForeignKey(FireReport, on_delete=models.CASCADE, related_name='vehicle_checklists', verbose_name='گزارش آتش‌نشانی')
    ...
    def __str__(self):
        ...
        vehicle_name = vehicle.vehicle_code if hasattr(vehicle, 'vehicle_code') else (vehicle.plate_number if hasattr(vehicle, 'plate_number') else 'نامشخص')
        return f'{self.fire_report} - {vehicle_name}'
